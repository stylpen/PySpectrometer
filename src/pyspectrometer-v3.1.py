...
class App:
	...
	def __init__(self, args, window, window_title):
		...
		self.video_source = args.index
		self.def_font = tkinter.font.nametofont("TkDefaultFont")
		self.def_font.config(size=9)
		self.vid = MyVideoCapture(args.calibration or App.DEFAULT_CALIBRATION, self.video_source, args.reference)
...
class MyVideoCapture:
	def __init__(self, calibration, video_source=0, reference=""):
		...
		if reference:
			with open(reference) as csvfile:
				...
				for row in reader:
					self.reference[0].append((float(row[0])))
					self.reference[1].append((int(row[1])))
		...
		'''
		self.vid.set(cv2.CAP_PROP_FRAME_WIDTH,2028)
		...
		if self.vid.isOpened():
			...
			if ret:
				...
				for i in range(cols-4):
					data = sum(bwimage[halfway-10:halfway+11,i])/21 # averaging 21px around center
				...
				self.wavelengthdata = []
				index=0
				while index < len(self.intensity):
					...
					#origin is top left.
					cv2.line(graph, (index,255), (index,255-self.intensity[index]), (r,g,b), 1)
					if index == 0:
						cv2.line(graph, (index, 254-self.intensity[index]), (index, 255-self.intensity[index]), (0,0,0), 1, cv2.LINE_AA)
					else:
						cv2.line(graph, (index-1, 255-self.intensity[index-1]), (index, 255-self.intensity[index]), (0,0,0), 1, cv2.LINE_AA)
					index+=1
				...
				if self.reference:
					...
					prev_y = 0
					index = 0
					while (index < len(self.reference[0]) and (index < len(self.reference[1]))):
						wavelength = self.reference[0][index]
						x = int(round(wavelength - scalezero) / nmperpx)
						if x < 0:
							index+=1
							continue
						wavelengthdata = self.reference[1][index]
						if prev_x == 0:
							cv2.line(graph, (x, 254-wavelengthdata), (x, 255-wavelengthdata), (42,84,84), 1, cv2.LINE_AA)
						else:
							cv2.line(graph, (prev_x, 255-prev_y), (x, 255-wavelengthdata), (42,84,84), 1, cv2.LINE_AA)
						prev_x = x
						prev_y = wavelengthdata
						index+=1
